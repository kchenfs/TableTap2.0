...
const getAppMode = (hostname: string) => {
  if (hostname.includes('take-out')) return 'takeout';
  return process.env.NEXT_PUBLIC_APP_MODE || 'dine-in';
};

const getTableId = (appMode: string) => {
  if (appMode === 'takeout') return 'takeout';
  const pathname = window.location.pathname;
  if (pathname.startsWith('/table-')) {
    const match = pathname.match(/\/table-(\d+)/);
    if (match && match[0]) return match[0].replace('/', '');
  }
  if (window.ENV?.TABLE_ID) return window.ENV.TABLE_ID;
  return process.env.NEXT_PUBLIC_TABLE_ID || 'table-1';
};

export default function MomotaroApp() {
  ...
  const [appMode, setAppMode] = useState('dine-in');
  const [tableId, setTableId] = useState('table-1');

  useEffect(() => {
    const mode = getAppMode(window.location.hostname);
    setAppMode(mode);
    setTableId(getTableId(mode));
  }, []);
